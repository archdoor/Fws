...
{
    // 与用户信息处理模块UDP接收接口
    m_users_rfd = GetUdpBindSocket(g_configs.clinker_ip.c_str(), g_configs.clinker_udp_port_for_users);
    if ( m_users_rfd < 0 )
    {
    ...
    // 与集中监测系统接口模块UDP接收接口
    m_interface_rfd = GetUdpBindSocket(g_configs.clinker_ip.c_str(), g_configs.clinker_udp_port_for_interface);
    ...
    // 与安全处理模块UDP发送接口
    m_safety_wfd = GetUdpSocket();
    if ( m_safety_wfd < 0 )
    {
        CloseSock(m_safety_wfd);
        LogError("GetUdpSocket error\n");
        return -1;
    }
    // 与安全处理模块UDP接收接口
    m_safety_rfd = GetUdpBindSocket(g_configs.clinker_ip.c_str(), g_configs.clinker_udp_port_for_safety);
    if ( m_safety_rfd < 0 )
    {
...
int Interactor::start()
{
    ...
    {   
        ...
        FD_SET(m_safety_rfd, &fd_read_set);
        ...
        else 
        {
            ...
            if ( FD_ISSET(m_safety_rfd, &fd_read_set) )
            {
                int framelen = recv_from_module_safety(buff, sizeof(buff));
                if ( framelen < 0 )
                {
                    LogWarning("recv_from_module_safety fail\n");
                }
                LogDebug("收到来自safety的信息帧\n");
                // worker.handle(buff, framelen);
            }
        }
    }
...
{
    return SendTo(m_users_wfd, g_configs.users_ip.c_str(), g_configs.users_udp_port_for_clinker, buff, len);
}
...
{
    return SendTo(m_safety_wfd, g_configs.clinker_ip.c_str(), g_configs.safety_udp_port_for_clinker, buff, len);
}
